...
function safeStorageExtension(filename: string) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const normalizedLessonIds = [
      ...new Set([...lessonIds, ...(lessonId ? [lessonId] : [])]),
    ];
    const normalizedTopicIds = [
      ...new Set([...topicIds, ...(topicId ? [topicId] : [])]),
    ];
    ...
    // Original human-readable filename stays in D1/UI.
    const name = cleanFilename(candidate.name);
    const mimeType = candidate.type || "application/octet-stream";

    // Supabase Storage keys are deliberately ASCII-only.
    // Never put the original filename into the object key: it may contain
    // Cyrillic, emoji or other characters rejected by Storage.
    ...
    const [row] = await db
      ...
      .values({
        ...
        lessonId,
        topicId,
        lessonIds: JSON.stringify(normalizedLessonIds),
        topicIds: JSON.stringify(normalizedTopicIds),
        scope,
        name,
        label,
        kind,
        mimeType,
        size: candidate.size,
        // The legacy D1 column name is kept for compatibility;
        // it stores the Supabase Storage object path.
        r2Key: storagePath,
      })
      .returning();
...
export async function GET(request: Request) {
  ...
  if (!id) {
    return Response.json({ error: "Не указан файл." }, { status: 400 });
  }
