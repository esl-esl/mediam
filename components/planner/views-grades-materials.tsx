...
import { formatSubjectModules } from "@/lib/planner-utils";
...
export function UploadDialog({ open, onOpenChange, defaultSubject = null, defaultLessonId = null, defaultTopicId = null, defaultLessonIds, defaultTopicIds, defaultScope, defaultKind = "file" }: {
  ...
  const { state, uploadMaterial } = usePlanner(); const subjects = Array.isArray(state.subjects) ? state.subjects : []; const [file, setFile] = React.useState<File | null>(null); const [subjectId, setSubjectId] = React.useState(defaultSubject ?? "none"); const [label, setLabel] = React.useState(""); const [kind, setKind] = React.useState<Material["kind"]>(defaultKind); const [loading, setLoading] = React.useState(false); const [lessonIds, setLessonIds] = React.useState<string[]>([]); const [topicIds, setTopicIds] = React.useState<string[]>([]);
...
export function MaterialsView() {
  ...
  const materials = sourceMaterials.filter((item) => (subject === "all" || item.subjectId === subject) && `${item.name} ${item.label}`.toLowerCase().includes(query.toLowerCase()));
