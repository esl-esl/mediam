...
export function MaterialDialog({ open, onOpenChange, defaultSubjectId = null, defaultLessonId = null, defaultTopicId = null, defaultLessonIds, defaultTopicIds, defaultScope, defaultKind, material }: {
  ...
  const { state, mutate } = usePlanner();
  const [name, setName] = React.useState(""); const [label, setLabel] = React.useState(""); const [url, setUrl] = React.useState("");
  ...
  React.useEffect(() => {
    if (!open) return;
    setName(material?.name ?? ""); setLabel(material?.label ?? ""); setUrl(material?.url ?? "");
  ...
  function submit(event: React.FormEvent) {
    event.preventDefault(); if (!name.trim() || !url.trim()) return;
    mutate((draft) => {
      const next: Material = { id: material?.id ?? uid("material"), subjectId: subjectId === "none" ? null : subjectId, lessonId: lessonIds[0] ?? null, topicId: topicIds[0] ?? null, lessonIds, topicIds, scope: material?.scope ?? defaultScope ?? (topicIds.length ? "topic" : lessonIds.length ? "lesson" : subjectId === "none" ? "general" : "subject"), name: name.trim(), label: label.trim() || "Ссылка", kind, storage: "link", url: url.trim(), createdAt: material?.createdAt ?? new Date().toISOString() };
      const index = draft.materials.findIndex((item) => item.id === next.id); if (index >= 0) draft.materials[index] = next; else draft.materials.push(next);
    });
    toast.success(material ? "Материал обновлён" : "Материал добавлен"); onOpenChange(false);
  }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><form onSubmit={submit} className="grid gap-4"><DialogHeader><DialogTitle>{material ? "Изменить материал" : "Добавить ссылку"}</DialogTitle></DialogHeader><Field label="Название"><Input value={name} onChange={(event) => setName(event.target.value)} autoFocus /></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="Предмет"><Select value={subjectId} onValueChange={(value) => { setSubjectId(value); setLessonIds([]); setTopicIds([]); }}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Без предмета</SelectItem>{state.subjects.map((subject) => <SelectItem key={subject.id} value={subject.id}>{subject.shortTitle}</SelectItem>)}</SelectContent></Select></Field><Field label="Тип"><Select value={kind} onValueChange={(value) => setKind(value as Material["kind"])}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="link">Полезная ссылка</SelectItem><SelectItem value="recording">Запись лекции</SelectItem><SelectItem value="presentation">Презентация</SelectItem><SelectItem value="textbook">Учебник</SelectItem><SelectItem value="gradebook">Ведомость</SelectItem></SelectContent></Select></Field></div><Field label="Подпись"><Input value={label} onChange={(event) => setLabel(event.target.value)} /></Field><Field label="URL"><Input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://" /></Field><RelationPicker subjectId={subjectId} lessonIds={lessonIds} topicIds={topicIds} onLessonIdsChange={setLessonIds} onTopicIdsChange={setTopicIds} /><DialogFooter><Button type="button" variant="ghost" onClick={() => onOpenChange(false)}>Отмена</Button><Button type="submit">Сохранить</Button></DialogFooter></form></DialogContent></Dialog>;
}
